...
**Originality requirement (applies to every prompt, permanently):** All code must be written fresh, specific to this project's actual schema, config, and endpoints — never copied verbatim from a tutorial, Stack Overflow answer, or another public repository. It is acceptable and expected for code to use standard, well-known library patterns (e.g. a typical FastAPI route, a standard pandas merge) since these are common idioms, not original creative work — but full functions or files should not be lifted wholesale from an external source. Every AI tool used on this project should be told: "Write original code for this specific project, not copied from an existing tutorial or repository."